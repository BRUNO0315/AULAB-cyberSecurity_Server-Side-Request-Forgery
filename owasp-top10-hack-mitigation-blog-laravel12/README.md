...
<!-- per migrare le vecchi psw dopo aver modificato la login ho usato i segeunti comandi:
...
$users = User::whereIn('id', [1,3,11])->get();
...
foreach ($users as $user) {
    ...
    $user->update([
        'salt' => $salt,
        'password' => Hash::make('password' . $salt . $pepper),
    ]);
}
