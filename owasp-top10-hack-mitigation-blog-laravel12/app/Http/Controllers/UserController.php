...
public function uploadFile(Request $request)
{
    ...
    $request->validate([
        'user_file' => 'required|file|mimes:pdf,jpg,jpeg,png,gif|max:10240'
    ]);
    ...
    if (!in_array($extension, $allowedExtensions)) {
        return back()->withErrors([
            'user_file' => 'File type not allowed'
        ]);
    }

    // Genera nome sicuro usando SHA256
    $fileUid = hash_file('sha256', $file)
        . '.'
        . $extension;

    // Salvataggio in cartella privata
    $file->storeAs(
        "docs/users/{$user->id}",
        $fileUid,
        'local'
